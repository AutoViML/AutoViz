...
def save_image_data(fig, filename, chart_format):
    if chart_format == 'svg':
...
def draw_pivot_tables(dft,cats,nums,problem_type,verbose,chart_format,depVar='', classes=None):
    ...
    if not os.path.isdir(mk_dir):
        os.mkdir(mk_dir)
    save_dir = os.path.join(".","AutoViz_Plots",target_dir,plot_name+"."+chart_format)
    ##################################################################################
    cats = list(set(cats))
    dft = dft[:]
    cols = 2
    ...
    if problem_type == 'Regression' or depVar==None or depVar=='' or depVar==[]:
        ...
        fig.tight_layout();
        if verbose == 2:
            imgdata_list.append(save_image_data(fig, save_dir, chart_format))
            image_count += 1
    else:
...
def draw_barplots(dft,cats,conti,problem_type,verbose,chart_format,dep='', classes=None):
    ...
    for k in range(len(cats)):
        ...
        if dep==None or dep == '':
            ...
            if not os.path.isdir(mk_dir):
                os.mkdir(mk_dir)
            save_dir = os.path.join(".","AutoViz_Plots",target_dir,f'BarPlots_{each_conti}_by_{cats[k]}'+"."+chart_format)
            ##################################################################################
            if verbose == 1:
                plt.show();
            ###########
...
def draw_violinplot(df, dep, nums,verbose,chart_format, modeltype='Regression'):
    ...
    if modeltype == 'Regression' or dep == None or dep == '':
        ...
        for row in range(rows):
            ...
            if not os.path.isdir(mk_dir):
                os.mkdir(mk_dir)
            save_dir = os.path.join(".","AutoViz_Plots",target_dir,plot_name+str(plot_index)+"."+chart_format)
            ##################################################################################
            if verbose == 1:
                plt.show();
            if verbose == 2:
