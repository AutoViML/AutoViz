...
def draw_pivot_tables(dft,cats,nums,problem_type,verbose,chart_format,depVar='', classes=None, save_dir=Path('./Plots/')):
    cats = list(set(cats))
    dft = dft[:]
    cols = 2
    ...
    if problem_type == 'Regression' or depVar==None or depVar=='' or depVar==[]:
        ...
        for i,color in zip(range(len(cats)), colors) :
            if len(dft[cats[i]].unique()) >= categorylimit:
                ...
                k += 1
        fig.tight_layout();
        fig.savefig(save_dir/'PivotPlots.png')
        if verbose == 2:
        ...
        if len(ls_cats) <= 2:
            ...
            for eachpred,color3 in zip(nums,colors):
                ...
                counter += 1
            fig.tight_layout();
            fig.savefig(save_dir/'PivotTables.png')
            if verbose == 2:
                imgdata_list.append(save_image_data(fig, image_count, chart_format))
                image_count += 1
        else:
...
def draw_scatters(dfin,nums,verbose,chart_format,problem_type,dep=None, classes=None, lowess=False, save_dir=Path('./Plots/')):
    dft = dfin[:]
    ...
    if dep == None or dep == '':
        ...
        fig.tight_layout();
        fig.savefig(save_dir/'ScatterPlots.png')
        if verbose == 1:
            plt.show();
...
def draw_pair_scatters(dfin,nums,problem_type, verbose,chart_format, dep=None, classes=None, lowess=False, save_dir=Path('./Plots/')):
    """
    ...
    if problem_type == 'Regression' or problem_type == 'Clustering':
        ...
        fig.suptitle('Pair-wise Scatter Plot of all Continuous Variables', fontsize=20,y=1.08)
        fig.tight_layout();
        fig.savefig(save_dir/'PairWiseScatterPlots.png')
        if verbose == 2:
            imgdata_list.append(save_image_data(fig, image_count, chart_format))
            image_count += 1
        if verbose == 1:
        ...
        fig.suptitle('Scatter Plot of each Continuous Variable against Target Variable', fontsize=20,y=1.08)
        fig.tight_layout();
        fig.savefig(save_dir/'PairScatterPlots.png')
        if verbose == 2:
            imgdata_list.append(save_image_data(fig, image_count, chart_format))
            image_count += 1
        if verbose == 1:
...
def draw_barplots(dft,cats,conti,problem_type,verbose,chart_format,dep='', classes=None, save_dir=Path('./Plots/')):
        ...
        if dep==None or dep == '':
            ...
            for each_conti,color in zip(conti,colors):
                ...
                if dft[cats[k]].nunique() > chunksize:
                    ...
                    kadd += 1
            fig.tight_layout();
            plt.savefig(save_dir/f'BarPlots Average_{each_conti}_by_{cats[k]}_(Ascending).png')
            if verbose == 1:
                plt.show();
            ###########
            if verbose == 2:
            ...
            for each_conti,color in zip(conti,colors):
                if len(dft[cats[k]].value_counts()) < 20:
                    ...
                    kadd += 1
            fig.tight_layout();
            plt.savefig(save_dir/f'BarPlots Mean_{each_conti}_by_{cats[k]}_(Ascending).png')
            if verbose == 2:
                imgdata_list.append(save_image_data(fig, image_count, chart_format))
                image_count += 1
            elif verbose == 1:
            ...
            plt.savefig(save_dir/'BarPlots Average_{each_conti}_by_{cats[k]}_(Ascending).png')
            if verbose == 2:
                imgdata_list.append(save_image_data(fig, image_count, chart_format))
                image_count += 1
            elif verbose == 1:
...
def draw_heatmap(dft, conti, verbose,chart_format,datevars=[], dep=None,
                                    modeltype='Regression',classes=None, save_dir=Path('./Plots/')):
    ...
    if modeltype != 'Regression':
        ...
        fig.tight_layout();
        fig.savefig(save_dir/'HeatMap.png')
        if verbose == 1:
            plt.show();
        if verbose == 2:
            imgdata_list.append(save_image_data(fig, image_count, chart_format))
...
def draw_distplot(dft, conti,verbose,chart_format,problem_type,dep=None, classes=None, save_dir=Path('./Plots/')):
    ...
    if dep==None or dep=='' or problem_type == 'Regression':
        ...
        #fig.tight_layout();
        plt.savefig(save_dir/'DistPlot.png')
        if verbose == 2:
            imgdata_list.append(save_image_data(fig, image_count, chart_format))
            image_count += 1
        ...
        for each_conti,k in zip(conti,range(len(conti))):
            ...
            ax1.legend(loc='best')
        fig.tight_layout();
        plt.savefig(save_dir/'DistPlots.png')
        if verbose == 1:
            plt.show();
        if verbose == 2:
            imgdata_list.append(save_image_data(fig, image_count, chart_format))
...
def draw_violinplot(df, dep, nums,verbose,chart_format, modeltype='Regression', save_dir=Path('./Plots/')):
    df = df[:]
...
def draw_date_vars(df,dep,datevars, num_vars,verbose, chart_format, modeltype='Regression', save_dir=Path('./Plots/')):
    ...
    if modeltype == 'Regression' or dep == None or dep == '':
        ...
        #fig.tight_layout();
        if verbose == 1:
            plt.savefig(save_dir/'TimeSeriesPlots.png')
            plt.show();
        if verbose == 2:
            imgdata_list.append(save_image_data(fig, image_count, chart_format))
            image_count += 1
